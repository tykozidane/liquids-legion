...
export async function POST(request: Request) {
    try {
        ...
        const result = await query(
            `UPDATE opr.finance SET ${dataUpdate}, updated_at = '${timeUpdate}', updated_by = '${checkJWT.data?.username} where id = '${product_id}'`,
          );

        return NextResponse.json({status: "00", message: "Update Data Succesfully", data: result}, {status: 200})
    } catch(err:any){
        console.log(err);
